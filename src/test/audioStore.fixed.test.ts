...
vi.mock('tone', () => {
  ...
  return {
    ...
    now: vi.fn(() => 0),
    Analyser: vi.fn().mockImplementation(() => ({ // Add missing Analyser
...
describe('AudioStore - Critical InvalidAccessError Fix', () => {
  ...
  describe('Critical Bug: StartTone InvalidAccessError', () => {
    ...
    it('should handle oscillator creation failure', async () => {
      ...
      await act(async () => {
        await expect(result.current.startTone()).rejects.toThrow('InvalidAccessError')
      })
    })
  ...
  describe('Audio Context State Management', () => {
    it('should check audio context state before operations', async () => {
      ...
      await act(async () => {
        await expect(result.current.startTone()).rejects.toThrow('Audio context not initialized')
      })
    })
