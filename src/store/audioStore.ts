...
export const useAudioStore = create<AudioStore>((set, get) => ({
  ...
  initializeAudio: async () => {
    try {
      ...
      try {
        ...
        console.warn(
          "⚠️ Failed to load output devices, but audio system is still functional:",
          deviceError
        );
  ...
  startTone: async (config?: Partial<AudioSettings>) => {
    ...
    try {
      ...
      console.log("🎵 System audio check:", {
        ...
        baseLatency:
          (Tone.getContext().rawContext as AudioContext).baseLatency ||
          "unknown",
      });
      ...
      console.log(
        `🎵 Creating tone with HIGH VOLUME: ${frequency}Hz, ${
          settings.waveform
      ...
      console.log(
        `✅ Tone configuration complete: ${frequency}Hz, ${
          settings.waveform
        }, ${Math.round(volume * 100)}%`
      );
  ...
  updateFrequency: (frequency: number, unit: "Hz" | "kHz" = "Hz") => {
    ...
    console.log(
      `🎵 Updating frequency: ${frequency}${unit} -> ${clampedFreq}Hz`
    );
  ...
  updateVolume: (volume: number) => {
    ...
    if (state.isPlaying && state.gainNode) {
      try {
        ...
        console.log(
          `✅ Live volume updated: ${Math.round(validatedVolume * 100)}%`
        );
      } catch (error) {
  ...
  validateVolume: (volume: number): number => {
    ...
    if (volume > maxVolume) {
      console.warn(
        `⚠️ Volume clamped from ${Math.round(volume * 100)}% to ${Math.round(
          maxVolume * 100
        )}%`
      );
      return maxVolume;
  ...
  loadOutputDevices: async () => {
    try {
      ...
      if (!navigator.mediaDevices.getUserMedia) {
        console.warn(
          "⚠️ getUserMedia not available, trying to enumerate devices without permissions..."
        );
        ...
        console.log(
          `🎵 Found ${audioOutputs.length} output devices (without labels):`,
          audioOutputs
        );
      ...
      try {
        ...
        console.warn(
          "⚠️ Microphone permission denied, device labels may not be available:",
          permissionError
        );
      ...
      console.log(
        `🎵 Found ${audioOutputs.length} output devices:`,
        audioOutputs
      );
  ...
  selectOutputDevice: async (deviceId: string) => {
    try {
      ...
      const state = get();
      const device = state.availableOutputDevices.find(
      ...
      if (typeof audioContext.setSinkId === "function") {
        try {
          ...
          console.log(
            `🎵 Set audio context sink to: ${device.label || "Unknown Device"}`
          );
        } catch (sinkError) {
        ...
        console.warn(
          "💡 Device selection recorded but may not affect audio output"
        );
      }

      set({ selectedOutputDevice: deviceId });
      console.log(
        `✅ Selected output device: ${device.label || "Unknown Device"}`
      );
    } catch (error) {
      ...
      if (error instanceof Error) {
        if (error.message.includes("not found")) {
          ...
          console.error(
            "💡 This browser may not support audio output device selection"
          );
        }
      }
